// Distributed as actualizar-keytube-sepolia/instalar.mjs. No packages or network required.
...
try {
  ...
  if(project.name!=='keytube'||!existsSync(path.join(root,'app/keytube-v2.tsx')))throw new Error('Coloca la carpeta actualizar-keytube-sepolia dentro de tu carpeta keytube actual, junto a package.json.');
  if(!existsSync(path.join(root,'node_modules/wrangler/bin/wrangler.js')))throw new Error('Esta carpeta no tiene las dependencias instaladas. Copia actualizar-keytube-sepolia dentro de la carpeta KeyTube-cuentas-y-planes/keytube que ya funciona.');
  ...
  console.log('SEPOLIA-LINK-7 instalada. Ejecuta pnpm.cmd build y, si termina sin errores, pnpm.cmd dev.');
}catch(error){
