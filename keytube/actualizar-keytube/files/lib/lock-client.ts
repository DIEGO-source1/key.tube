...
export async function prepareWallet(network:number,account:Address) {
  const provider=await requireWalletAccount(account);
  const {chain}=chainConfig(network);
  try {await provider.request({method:'wallet_switchEthereumChain',params:[{chainId:`0x${network.toString(16)}`}]});}
  catch(e){
    if((e as {code?:number}).code!==4902)throw e;
    await provider.request({method:'wallet_addEthereumChain',params:[{chainId:`0x${network.toString(16)}`,chainName:chain.name,nativeCurrency:chain.nativeCurrency,rpcUrls:[...chain.rpcUrls.default.http],blockExplorerUrls:[chain.blockExplorers.default.url]}]});
  }
  if(await requireWalletAccount(account)!==provider)throw new Error('La wallet cambió. Vuelve a conectar antes de continuar.');
