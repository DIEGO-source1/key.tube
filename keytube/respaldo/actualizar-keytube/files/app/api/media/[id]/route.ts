...
export async function GET(
  ...
  try {
    ...
    if (url.searchParams.get("owner") === "1") {
      ...
      const published = await db().prepare("SELECT id FROM posts WHERE id=? AND asset_id=? AND visibility='free' AND status='published'").bind(url.searchParams.get("public"),asset.id).first();
      ...
      const post = await db()
        .prepare("SELECT * FROM posts WHERE id = ? AND asset_id = ? AND status='published'")
    ...
    } else if (asset.role === "avatar") {
      const published = await db().prepare("SELECT owner_id FROM profiles WHERE owner_id=? AND avatar=?").bind(asset.owner_id,id).first();
      if (!published) throw new AppError(404,"Esta foto de perfil no está publicada.");
    } else {
      const published = await db()
        .prepare(
          "SELECT id FROM posts WHERE (thumbnail_id = ? OR preview_id = ?) AND status='published' LIMIT 1",
        )
        .bind(id, id)
        .first();
      if (!published)
