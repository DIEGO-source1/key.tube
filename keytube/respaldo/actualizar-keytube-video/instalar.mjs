// Distributed as actualizar-keytube-video/instalar.mjs. No packages or network required.
...
try {
  ...
  if(project.name!=='keytube'||!existsSync(path.join(root,'app/keytube-v2.tsx')))throw new Error('Coloca la carpeta actualizar-keytube-video dentro de tu carpeta keytube actual, junto a package.json.');
  if(!existsSync(path.join(root,'node_modules/wrangler/bin/wrangler.js')))throw new Error('Esta carpeta no tiene las dependencias instaladas. Copia actualizar-keytube-video dentro de la carpeta KeyTube-cuentas-y-planes/keytube que ya funciona.');
  if(!existsSync(path.join(root,'lib/lock-reference.ts')))throw new Error('Instala primero la actualización KeyTube-vincular-Sepolia.zip.');
  ...
  console.log('VIDEO-PREVIEW-8 instalada. Ejecuta pnpm.cmd build y después node .\\scripts\\start-local.mjs.');
}catch(error){
