...
import { mainnet, base, baseSepolia, sepolia, polygon } from "viem/chains";
import type { Membership } from "./keytube-types";
import { UnlockError } from './unlock-errors';
const networks = {
  [mainnet.id]: {
    chain: mainnet,
    factory: "0xe79B93f8E22676774F2A8dAd469175ebd00029FA",
  },
  [baseSepolia.id]: {
    chain: baseSepolia,
...
export function rpcClient(network: number) {
  ...
  // PublicNode's documented Sepolia endpoint, with the existing provider as a
  // fallback. All reads remain on the selected chain and fail closed.
  const urls = network === sepolia.id
    ? ['https://ethereum-sepolia-rpc.publicnode.com', ...chain.rpcUrls.default.http]
    : [...chain.rpcUrls.default.http];
  return createPublicClient({
    chain,
    transport: network === sepolia.id
      ? fallback(urls.map(url => http(url, {timeout: 7000, retryCount: 0})), {retryCount: 0})
      : http(chain.rpcUrls.default.http[0], {timeout: 12000, retryCount: 1}),
  });
}
export async function verifyRealLock(lock: Address, network: number) {
  ...
  let factory: Address, registration: readonly [boolean, bigint, bigint];
  try {
    [factory, registration] = await Promise.all([
    client.readContract({
      address: lock,
      abi: lockAbi,
    ...
    const noContract = error instanceof BaseError && error.walk(e =>
      e instanceof ContractFunctionZeroDataError || e instanceof ContractFunctionRevertedError);
    if (noContract instanceof ContractFunctionZeroDataError || noContract instanceof ContractFunctionRevertedError)
      throw new UnlockError(`No encontramos un Lock válido en ${config.chain.name}. Pega el enlace completo de tu Lock en Unlock para seleccionar su red automáticamente.`, 400, 'LOCK_NOT_FOUND');
    throw new UnlockError(`No pudimos consultar ${config.chain.name}. La conexión con la red no respondió. Vuelve a intentarlo en unos momentos.`, 503, 'RPC_UNAVAILABLE');
  }
  if (
