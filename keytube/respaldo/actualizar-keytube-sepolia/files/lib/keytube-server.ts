...
import { z } from "zod";
import { isAddress, BaseError, type Address, type Hex } from "viem";
import { findUnlockError } from './unlock-errors';
...
export function failure(error: unknown) {
  const unlockError = findUnlockError(error);
  if (unlockError) return response({error: unlockError.message, code: unlockError.code}, unlockError.status);
  if (error instanceof BaseError)
    return response({error: 'No pudimos verificar el contrato o la firma con la red seleccionada. Comprueba la conexión e inténtalo de nuevo.', code: 'CHAIN_REQUEST_FAILED'}, 503);
