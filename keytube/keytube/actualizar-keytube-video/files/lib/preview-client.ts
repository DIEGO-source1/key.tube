...
import { trimWebmPreview } from './webm-preview';
...
export async function videoPreview(file:File,onProgress:(seconds:number)=>void) {
  ...
  try {
    ...
    const cap=Math.min(9,Number.isFinite(video.duration)?video.duration*.9:9);
    ...
    // Check the encoded timestamps, not only the playback/JavaScript timer.
    // This also gives the WebM an explicit duration for browser playback.
    onProgress(Math.min(10,cap));
    const short=trimWebmPreview(new Uint8Array(await blob.arrayBuffer()),cap);
    return {preview:new File([new Uint8Array(short)],'adelanto-10s.webm',{type:'video/webm'}),cover};
  }finally {
